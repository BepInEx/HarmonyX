...
namespace HarmonyLib
{
    internal static class HarmonySharedState
    {
        ...
        private static Dictionary<MethodBase, byte[]> GetState()
        {
            lock (name)
            {
                ...
                if (assembly == null)
                {
                    ...
                    typeBuilder.DefineField("state", typeof(Dictionary<MethodBase, byte[]>),
                                            FieldAttributes.Static | FieldAttributes.Public);
                    typeBuilder.DefineField("version", typeof(int), FieldAttributes.Static | FieldAttributes.Public)
                               .SetConstant(internalVersion);
        ...
        {
            return AppDomain.CurrentDomain.GetAssemblies().FirstOrDefault(a => a.GetName().Name.Contains(name));
        }
