...
namespace HarmonyLib.Internal
{
    ...
    {
        private static readonly string upgradeToLatestVersionFullName = typeof(UpgradeToLatestVersion).FullName;

        private static bool IsUpgrade(object attribute)
        {
        ...
        {
            var attribute = method.GetCustomAttributes(false).FirstOrDefault(IsUpgrade);
            if (attribute == null)
                return -1;
        ...
        private static bool IsHarmonyAssembly(Assembly assembly)
        {
            try
            {
                return assembly.ReflectionOnly == false && assembly.GetType(typeof(HarmonyLib.Harmony).FullName) != null;
            }
            catch
            {
                return false;
        ...
        private static List<MethodBase> GetAllMethods(Assembly assembly)
        {
            ...
            return types.SelectMany(type => type.GetMethods(AccessTools.all).Cast<MethodBase>())
                        .Concat(types.SelectMany(type => type.GetConstructors(AccessTools.all)).Cast<MethodBase>())
                        .Concat(types.SelectMany(type => type.GetProperties(AccessTools.all))
                                     .Select(prop => prop.GetGetMethod()).Cast<MethodBase>())
                        .Concat(types.SelectMany(type => type.GetProperties(AccessTools.all))
                                     .Select(prop => prop.GetSetMethod()).Cast<MethodBase>())
                        .Where(method => method != null && method.DeclaringType.Assembly == assembly)
                        .OrderBy(method => method.FullDescription()).ToList();
        }
        ...
        internal static void PatchOldHarmonyMethods()
        {
            ...
            if (HarmonyLib.Harmony.DEBUG)
            {
                ...
                potentialMethodsToUpgrade.Values.OrderBy(method => method.FullDescription())
                                         .Do(method => FileLog.Log("- " + method.FullDescription()));
            }
                        ...
                        if (oldVersion < newVersion)
                        {
                            ...
                            if (generics != null)
                            {
                                foreach (var generic in generics)
                                {
                                    ...
                                    if (HarmonyLib.Harmony.DEBUG)
                                        FileLog.Log("Self-patching " + oldMethodInfo.FullDescription() + " with <" +
                                                    generic.FullName + "> in " + AssemblyInfo(assembly));
                            ...
                            {
                                if (HarmonyLib.Harmony.DEBUG)
                                    FileLog.Log("Self-patching " + oldMethod.FullDescription() + " in " +
            ...
            if (HarmonyLib.Harmony.DEBUG)
                FileLog.Log("Self-patched " + patchedCounter + " out of " + totalCounter + " methods (" +
                            (potentialCounter - patchedCounter) + " skipped) in " + watch.ElapsedMilliseconds + "ms");
        }
    }
}