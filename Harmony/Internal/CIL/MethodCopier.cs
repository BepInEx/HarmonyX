    ...
    internal class MethodBodyReader
    {
        ...
        internal void FinalizeILCodes(List<MethodInfo> transpilers, List<Label> endLabels)
        {
            ...
            codeInstructions.Do(codeInstruction =>
            {
                ...
                //
                codeInstruction.blocks.Do(block => { Emitter.MarkBlockBefore(generator, block, out var label); });
                    ...
                    switch (code.OperandType)
                    {
                        ...
                        case OperandType.InlineSig:
                            ...
                            if (operand is int == false)
                                throw new Exception("Wrong Emit argument type " + operand.GetType() + " in " +
                                                    codeInstruction);
        ...
        private ILInstruction GetInstruction(int offset, bool isEndOfInstruction)
        {
            ...
            if (offset < 0 || offset > ilInstructions[lastInstructionIndex].offset)
                throw new Exception("Instruction offset " + offset + " is outside valid range 0 - " +
                                    ilInstructions[lastInstructionIndex].offset);
        ...
        private OpCode ReadOpCode()
        {
            ...
            return op != 0xfe ? one_byte_opcodes[op] : two_bytes_opcodes[ilBytes.ReadByte()];
        }
        ...
        static MethodBodyReader()
        {
            ...
            typeof(ILGenerator).GetMethods().ToList().Do(method =>
            {
